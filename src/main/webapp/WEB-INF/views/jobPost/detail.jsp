...
<body>
	<div class="job-detail-container">
		<h1>${jobPost.title}</h1>
		<div class="job-info">
			<strong>ID:</strong> <span>${jobPost.jobPostId}</span> <strong>회사:</strong>
			<span>${jobPost.compId}</span> <strong>내용:</strong> <span>${jobPost.content}</span>
			<strong>직위:</strong> <span>${jobPost.position}</span> <strong>연봉:</strong>
			<span>${jobPost.salary}</span> <strong>경력:</strong> <span>${jobPost.experience}</span>
			<strong>지역:</strong> <span>${jobPost.location}</span> <strong>기능:</strong>
			<span>${jobPost.job}</span> <strong>직업:</strong> <span>${jobPost.function}</span>
						...
						<c:choose>
							<c:when test="${jobSeekerScrapVo.scrapId > 0}">
								<i id="scrapIcon" class="fa-solid fa-star on-Clicked"
									onclick="toggleScrap('${jobPost.jobPostId}', '${jobSeekerScrapVo.scrapId}')"></i>
							</c:when>
							<c:otherwise>
								<i id="scrapIcon" class="fa-regular fa-star"
									onclick="toggleScrap('${jobPost.jobPostId}', 0)"></i>
							</c:otherwise>
						</c:choose>
					</c:when>
					<c:otherwise>
